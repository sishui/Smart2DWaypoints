...
using System.IO;
using LitJson;
using Path = Smart2DWaypoints.Path;
    ...
    public class PathEditor : Editor
    {
        ...
        private void DrawWaypointsGUI()
        {
            if (_path.Waypoints.Any())
            {
                ...
                if (GUILayout.Button("Export Point", GUILayout.Width(100)))
                {
                    ExportWayPoints();
                }
                if(GUILayout.Button("Export Distance",GUILayout.Width(100)))
                {
                    ExportOnceDistance();
                }
                ...
                for (int i = 0; i < _path.Waypoints.Count; i++)
                {
                    ...
                    _path.Waypoints[i] =
                        EditorGUILayout.ObjectField(waypoint, typeof(Transform), true, GUILayout.Width(120)) as
                            Transform;
        ...
        {
            List<MovePoint> movePoints = _path.GetMovePoints();
            ...
            PathConfigSerializable result = new PathConfigSerializable {MovePoints = configSerialize};
            var filename = "Assets/art/game/fish/Track/" + _path.name + ".bytes";
            TextWriter tw = new StreamWriter(filename);
            string jsonStr = JsonMapper.ToJson(result);
            tw.Write(jsonStr);
            tw.Flush();
            tw.Close();
        }

        private void ExportOnceDistance()
        {
            var filename = "Assets/art/game/fish/Track/track" + _path.name + ".json";
            TextWriter tw = new StreamWriter(filename);
            string jsonStr = JsonMapper.ToJson(new PathServer{ id =_path.name, distance = _path.GetOnceDistance()});
            tw.Write(jsonStr);
            tw.Flush();
            tw.Close();
        }
        ...
        private void SetPositionForNewWaypoint(Transform newWaypoint, int ind)
        {
            ...
            else if (ind == _path.Waypoints.Count)
            {
                newWaypoint.position =
                    1.5f * _path.Waypoints[ind - 1].position - _path.Waypoints[ind - 2].position / 2f;
            }
            else if (ind == 0)
            {
