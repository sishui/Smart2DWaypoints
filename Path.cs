...
namespace Smart2DWaypoints
{
    ...
    {
        private readonly Dictionary<WaypointsMover, List<Vector3>> _moversParts =
            new Dictionary<WaypointsMover, List<Vector3>>();

        private readonly Dictionary<WaypointsMover, Dictionary<int, Vector3>> _capturedPositions =
            new Dictionary<WaypointsMover, Dictionary<int, Vector3>>();

        public List<Vector3> BezierPoints = new List<Vector3>();
        public List<Transform> Waypoints = new List<Transform>();
        public bool IsCurved;
        public int LinesCount = 100;
        ...
        public void OnDrawGizmos()
        {
            ...
            if (Selection.activeGameObject != gameObject)
            {
                foreach (Transform waypoint in Waypoints)
                    Gizmos.DrawSphere(waypoint.transform.position,
                        HandleUtility.GetHandleSize(transform.position) * 0.05f);
            }
#endif
        }
        ...
        public List<MovePoint> GetMovePoints()
        {
            ...
            for (int i = 0; i < points.Count; i++)
            {
                if (i < points.Count - 1)
                {
                    ...
                    movePoints.Add(new MovePoint(points[i], angle));
                }
                else
                {
                    movePoints.Add(new MovePoint(points[i], 0));
                }
            }
        ...
        public float GetOnceDistance()
        {
            List<MovePoint> movePoints = this.GetMovePoints();
            var distance = 0f;
            for (int i = 1; i < movePoints.Count; i++)
            {
                distance += Vector2.Distance(movePoints[i - 1].Position, movePoints[i].Position);
            }
